    ...
    if (empty($title) || empty($description) || $subject_id <= 0 || empty($format)) {
        ...
        if ($format == 'In-Person' && empty($location)) {
            ...
            if (mysqli_query($conn, $query)) {
                ...
                $error = "Failed to update tutorial: " . mysqli_error($conn);
            }
        }
    }
}
...
<div class="bg-gray-800">
    <div class="container mx-auto px-4 py-6">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold text-white">Edit Tutorial</h1>
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">Edit Tutorial</h2>
        ...
        <form method="POST" action="" id="tutorialForm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" value="<?php echo $tutorial['title']; ?>" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                </div>
                ...
                <div>
                    ...
                    <select id="subject_id" name="subject_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        ...
                        <?php foreach ($subjects as $subject): ?>
                            <option value="<?php echo $subject['id']; ?>" <?php echo ($tutorial['subject_id'] == $subject['id']) ? 'selected' : ''; ?>>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required><?php echo $tutorial['description']; ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="format" name="format" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        ...
                        <option value="Online" <?php echo ($tutorial['format'] == 'Online') ? 'selected' : ''; ?>>Online</option>
                        <option value="In-Person" <?php echo ($tutorial['format'] == 'In-Person') ? 'selected' : ''; ?>>In-Person</option>
                    </select>
                </div>
                
                <div id="locationField" style="display: <?php echo ($tutorial['format'] == 'In-Person') ? 'block' : 'none'; ?>">
                    <label for="location" class="block text-gray-700 font-medium mb-2">Location <span class="text-red-600">*</span></label>
                    <input type="text" id="location" name="location" value="<?php echo $tutorial['location']; ?>" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                
                <div id="fileUrlField" style="display: <?php echo ($tutorial['format'] == 'Online') ? 'block' : 'none'; ?>">
                    <label for="file_url" class="block text-gray-700 font-medium mb-2">File/Video URL</label>
                    <input type="text" id="file_url" name="file_url" value="<?php echo $tutorial['file_url']; ?>" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
